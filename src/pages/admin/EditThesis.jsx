...
import { Save, AlertCircle, ArrowLeft, PenTool } from 'lucide-react'

export default function EditThesis() {
  const { id } = useParams() // URL'den tez numarasını al
  ...
  // 1. Dropdown verilerini ve 2. Mevcut Tez verisini çek
  useEffect(() => {
    async function verileriGetir() {
      try {
        ...
        setHata('Veri yüklenemedi: ' + err.message)
      } finally {
        setYukleniyor(false)
      }
    }
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          approval_status: formData.approval_status // Statüyü de güncelleyebiliriz
        })
        .eq('thesis_no', id)

      if (error) throw error

      alert('Tez başarıyla güncellendi.')
      navigate('/admin/theses') // Listeye geri dön
    } catch (err) {
  ...
  if (yukleniyor) return <div className="text-center p-20">Yükleniyor...</div>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-[#faf7f2] shadow-2xl rounded-xl overflow-hidden border-2 border-stone-300">
        <div className="bg-amber-900 p-6 flex items-center gap-4 text-amber-50">
          ...
          <div>
            <h1 className="text-2xl font-bold font-serif">Tez Düzenle</h1>
            <p className="text-amber-200 text-sm">#{id} numaralı tezi düzenliyorsunuz.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleUpdate} className="p-8 space-y-6">
          ...
          {/* Statü Ayarı (Sadece Adminde Görünür) */}
          <div className="bg-amber-50 p-4 rounded border border-amber-200">
            <label className="block text-sm font-bold text-amber-900 mb-2">Yayın Durumu</label>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div>
              <label className="block text-sm font-bold text-stone-700 mb-1">Özet</label>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             {/* Kısa alanlar */}
             <div>
                <label className="block text-sm font-bold text-stone-700 mb-1">Yıl</label>
             ...
             <div>
                <label className="block text-sm font-bold text-stone-700 mb-1">Sayfa</label>
             ...
             <div className="col-span-2">
                <label className="block text-sm font-bold text-stone-700 mb-1">Tür</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-stone-200">
              {/* Dropdownlar */}
              <div>
                <label className="block text-sm font-bold text-stone-700 mb-1">Enstitü</label>
              ...
              <div>
                <label className="block text-sm font-bold text-stone-700 mb-1">Dil</label>
              ...
              <div>
                <label className="block text-sm font-bold text-stone-700 mb-1">Yazar</label>
              ...
              <div>
                <label className="block text-sm font-bold text-stone-700 mb-1">Danışman</label>

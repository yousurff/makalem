...
export default function Dashboard() {
  ...
  const tezOnayla = async (thesis_no) => {
    if (!window.confirm('Bu tezi yayınlamak istediğinize emin misiniz?')) return
    try {
      ...
      setInceleTez(null)
      alert('Tez başarıyla yayınlandı!')
    } catch (err) { alert('Hata: ' + err.message) }
  }

// Tez Reddetme (GÜNCELLENMİŞ HALİ)
  const tezReddet = async (thesis_no) => {
    if (!window.confirm('Bu tezi kalıcı olarak silmek istediğinize emin misiniz?')) return

    try {
      // ADIM 1: Önce teze bağlı anahtar kelimeleri ve konuları sil (Temizlik)
      const { error: kError } = await supabase.from('thesis_keyword').delete().eq('thesis_no', thesis_no)
      if (kError) console.log('Keyword silme uyarısı:', kError) // Hata olsa bile devam etmeye çalışabilir

      const { error: tError } = await supabase.from('thesis_subject_topic').delete().eq('thesis_no', thesis_no)
      if (tError) console.log('Topic silme uyarısı:', tError)

      // ADIM 2: Şimdi artık tezi silebiliriz
      const { error } = await supabase.from('thesis').delete().eq('thesis_no', thesis_no)

      if (error) throw error

      setBekleyenTezler(bekleyenTezler.filter(t => t.thesis_no !== thesis_no))
      setInceleTez(null) // Modalı kapat
      alert('Tez ve tüm bağlı verileri başarıyla silindi.')
    } catch (err) {
  ...
  if (yukleniyor) return <div className="text-center p-20 text-amber-900">Yükleniyor...</div>

  return (
    <div className="max-w-6xl mx-auto relative">
      <div className="flex justify-between items-center mb-8 border-b-2 border-stone-300 pb-4">
        <div>
          <h1 className="text-3xl font-bold text-amber-950 font-serif">Yönetim Masası</h1>
          <p className="text-stone-600">Sistem yönetimi ve onay bekleyen eserler</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <button onClick={() => navigate('/admin/universities')} className="bg-white p-4 rounded-lg border-2 border-stone-300 hover:border-amber-600 hover:text-amber-900 transition font-bold text-stone-600 shadow-sm flex flex-col items-center gap-2 group">
            <Building2 size={28} className="text-amber-800 group-hover:scale-110 transition-transform"/> Üniversiteler
        </button>
        <button onClick={() => navigate('/admin/institutes')} className="bg-white p-4 rounded-lg border-2 border-stone-300 hover:border-amber-600 hover:text-amber-900 transition font-bold text-stone-600 shadow-sm flex flex-col items-center gap-2 group">
            <Landmark size={28} className="text-amber-800 group-hover:scale-110 transition-transform"/> Enstitüler
        </button>
        <button onClick={() => navigate('/admin/people')} className="bg-white p-4 rounded-lg border-2 border-stone-300 hover:border-amber-600 hover:text-amber-900 transition font-bold text-stone-600 shadow-sm flex flex-col items-center gap-2 group">
            <Users size={28} className="text-amber-800 group-hover:scale-110 transition-transform"/> Kişiler
        </button>
        <button onClick={() => navigate('/admin/languages')} className="bg-white p-4 rounded-lg border-2 border-stone-300 hover:border-amber-600 hover:text-amber-900 transition font-bold text-stone-600 shadow-sm flex flex-col items-center gap-2 group">
            <Languages size={28} className="text-amber-800 group-hover:scale-110 transition-transform"/> Diller
        </button>
      </div>

      <h3 className="text-xl font-bold text-amber-950 mb-4 border-b-2 border-stone-300 pb-2">Onay Bekleyen Eserler</h3>
      ...
      {bekleyenTezler.length === 0 ? (
        <div className="bg-[#faf7f2] p-12 text-center rounded-xl border-2 border-dashed border-stone-300">
          ...
          <h3 className="text-xl font-bold text-stone-600">İncelenecek yeni tez yok.</h3>
          <p className="text-stone-500">Tüm başvurular değerlendirildi.</p>
        </div>
      ) : (
          ...
          {bekleyenTezler.map((tez) => (
            <div key={tez.thesis_no} className="bg-[#faf7f2] p-6 rounded-xl border border-stone-300 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => setInceleTez(tez)} className="flex items-center gap-1 bg-blue-100 text-blue-800 px-4 py-2 rounded shadow hover:bg-blue-200 transition font-bold text-sm border border-blue-200"><Eye size={16} /> İncele</button>
                <button onClick={() => tezOnayla(tez.thesis_no)} className="flex items-center gap-1 bg-green-700 text-white px-4 py-2 rounded shadow hover:bg-green-800 transition font-bold text-sm"><CheckCircle size={16} /> Yayınla</button>
                <button onClick={() => tezReddet(tez.thesis_no)} className="flex items-center gap-1 bg-red-700 text-white px-4 py-2 rounded shadow hover:bg-red-800 transition font-bold text-sm"><XCircle size={16} /> Reddet</button>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="bg-stone-200 p-5 border-b border-stone-300 flex justify-between items-center sticky top-0 z-10">
              <div>
                 <h3 className="font-bold text-stone-800 text-lg flex items-center gap-2">
                   <BookOpen size={24} className="text-amber-900"/> Tez İnceleme Dosyası
                 </h3>
            ...
            <div className="p-8 overflow-y-auto space-y-8">
              ...
              <div className="text-center border-b-2 border-stone-300 pb-6">
                <span className={`inline-block px-4 py-1 mb-3 rounded-full text-sm font-bold tracking-wider ${inceleTez.type === 'PhD' ? 'bg-purple-100 text-purple-900' : 'bg-green-100 text-green-900'}`}>
                    {inceleTez.type === 'PhD' ? 'DOKTORA TEZİ' : 'YÜKSEK LİSANS TEZİ'}
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white p-6 rounded-lg border border-stone-200 shadow-sm">
                 <div className="text-center p-2 border-r border-stone-100 md:border-stone-200 last:border-0">
                    <span className="block text-xs text-stone-400 font-bold uppercase mb-1">Eser Sahibi (Yazar)</span>
                    <span className="block font-bold text-stone-900 text-lg">{inceleTez.author?.first_name} {inceleTez.author?.last_name}</span>
                 </div>
                 <div className="text-center p-2 border-r border-stone-100 md:border-stone-200 last:border-0">
                    <span className="block text-xs text-stone-400 font-bold uppercase mb-1">Tez Danışmanı</span>
                 ...
                 <div className="text-center p-2">
                    <span className="block text-xs text-stone-400 font-bold uppercase mb-1">Eş Danışman</span>
                    <span className={`block font-bold text-lg ${inceleTez.co_supervisor ? 'text-stone-900' : 'text-stone-300 italic'}`}>
                        {inceleTez.co_supervisor ? `${inceleTez.co_supervisor.first_name} ${inceleTez.co_supervisor.last_name}` : 'Belirtilmemiş'}
                    </span>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 <div className="bg-stone-100 p-4 rounded border border-stone-200">
                    <span className="block text-xs text-stone-500 font-bold uppercase">Üniversite</span>
                    <span className="font-semibold text-stone-900">{inceleTez.institute?.university?.university_name}</span>
                 </div>
                 <div className="bg-stone-100 p-4 rounded border border-stone-200">
                    <span className="block text-xs text-stone-500 font-bold uppercase">Enstitü</span>
                    <span className="font-semibold text-stone-900">{inceleTez.institute?.institute_name}</span>
                 </div>
                 <div className="bg-stone-100 p-4 rounded border border-stone-200">
                    <span className="block text-xs text-stone-500 font-bold uppercase">Dil / Sayfa</span>
                    <span className="font-semibold text-stone-900">{inceleTez.language?.language_name} / {inceleTez.num_of_pages} sf.</span>
                 </div>
                 <div className="bg-stone-100 p-4 rounded border border-stone-200">
                    <span className="block text-xs text-stone-500 font-bold uppercase">Teslim Tarihi</span>
              ...
              {/* Özet */}
              <div>
                <h4 className="font-bold text-stone-800 border-b border-stone-300 mb-3 flex items-center gap-2"><FileText size={18}/> Özet</h4>
                <div className="bg-white p-4 rounded border border-stone-200 text-stone-700 text-justify leading-relaxed text-sm">
                    {inceleTez.abstract}
                </div>
              </div>

              {/* Anahtar Kelimeler ve Konular */}
              <div className="grid md:grid-cols-2 gap-8">
                  <div>
                    <h4 className="font-bold text-stone-800 text-xs uppercase mb-3 border-b border-stone-200 pb-1">Anahtar Kelimeler</h4>
                    {inceleTez.thesis_keyword && inceleTez.thesis_keyword.length > 0 ? (
                        ...
                        <span className="text-stone-400 italic text-sm">Anahtar kelime girilmemiş.</span>
                    )}
                  </div>
                  <div>
                    <h4 className="font-bold text-stone-800 text-xs uppercase mb-3 border-b border-stone-200 pb-1">Konu Alanları</h4>
                    {inceleTez.thesis_subject_topic && inceleTez.thesis_subject_topic.length > 0 ? (
                        ...
                        {inceleTez.thesis_subject_topic.map((t, i) => (
                         ...
                         <span className="text-stone-400 italic text-sm">Konu alanı seçilmemiş.</span>
                    )}
                  </div>
              </div>
